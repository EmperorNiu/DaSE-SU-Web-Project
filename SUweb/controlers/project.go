...
func GetProjectList(c *gin.Context) {
	...
	if err := models.QueryProjectList(&projects); err != nil {
		...
		var projList []models.ProjectIntro
		for _, x := range projects {
			projList = append(projList, models.ProjectIntro{
		...
		c.JSON(http.StatusOK, gin.H{"message": "success", "proj": projList})
	}
}

func GetProject(c *gin.Context) {
	var project models.Project
	project_id := c.DefaultQuery("userid", "0")
